...
if ( !class_exists('CoolVideoGallery') ) {
	...
	class CoolVideoGallery{
		...
		function _cvg_activate() {
			...
			$options_player['cvgplayer_navigation_controls'] = 0;
		...
		function admin_menu() {
			...
			add_submenu_page( 'cvg-gallery-overview', __('About the Author'), 'About Author', 'manage_options', 'cvg-plugin-about',array(&$this, 'about_plugin_author'));
		}
		...
		function CVGVideo_Render($matches){
			...
			if(isset($arguments['mode']) && $arguments['mode'] == "playlist") {
				...
				$video_display .= '</div>';
					...
					jQuery(document).ready(function(){
						...
						jwplayer("<?php echo "mediaplayer_vid_".$arguments['videoId']; ?>").setup({
							...
							"flashplayer" : "<?php echo $this->plugin_url . "cvg-player/player.swf"; ?>",
				...
				$output .=  'alt="' . htmlspecialchars('Click to Watch Video') . '" id="fancy_cvg_video_preview_'. $arguments["videoId"] . '"  /></a>';
				?>
		...
		function addPlayerHeader(){
			...
			<!-- Cool Video Gallery Script starts here -->
			<script type="text/javascript" src="<?php echo $this->video_player_url ?>jwplayer.js"></script>
			<script type="text/javascript" src="<?php echo $this->plugin_url ?>third_party_lib/fancybox/jquery.fancybox-1.3.4.pack.js"></script>
			<link rel="stylesheet" type="text/css" href="<?php echo $this->plugin_url ?>third_party_lib/fancybox/jquery.fancybox-1.3.4.css" media="screen" />
		
			<?php
				if($options_settings['cvg_navigation_controls'] == 0) {
				?>
