...
class CvgCore extends videoDB{
	...
	function process_youtube_videos($galleryID, $videolist) {
		...
		if ( is_array($videolist) ) {
			foreach($videolist as $video) {
				...
				$result = $wpdb->query( $wpdb->prepare("INSERT INTO " . $wpdb->prefix ."cvg_videos (galleryid, filename, thumb_filename, alttext, description, videodate, video_type, meta_data) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", $galleryID, $video_details->watchURL, $thumb_filename, $alttext, $wpdb->escape($video_details->description), $time_updated, $cool_video_gallery->video_type_youtube, serialize($meta)) );
	...
	function add_media_videos() {
		...
		// save it to the database 
		$result = $wpdb->query( $wpdb->prepare("INSERT INTO " . $wpdb->prefix ."cvg_videos (galleryid, filename, thumb_filename, alttext, description, videodate, video_type) VALUES (%s, %s, %s, %s, %s, %s, %s)", $galleryID, $post_details->guid, $thumbs_file_name, $alttext, $wpdb->escape($post_details->post_content), $time_updated, $cool_video_gallery->video_type_media) );
	...
	function add_Videos($galleryID, $videolist) {
		...
		if ( is_array($videolist) ) {
			foreach($videolist as $video) {
				...
				// save it to the database 
				$result = $wpdb->query( $wpdb->prepare("INSERT INTO " . $wpdb->prefix ."cvg_videos (galleryid, filename, thumb_filename, alttext, description, videodate, video_type) VALUES (%s, %s, %s, %s, %s, %s, %s)", $galleryID, $video, $thumb_filename, $alttext, $alttext, $time_updated, $cool_video_gallery->video_type_upload) );
				// and give me the new id
				$vid_id = (int) $wpdb->insert_id;
	...
	function xml_playlist($galleryID = null) {
		...
		foreach($gallery_details as $video){
			...
			$xml .= '<media:thumbnail url="' . $thumb_url . '" />';
	...
	function xml_sitemap() {
		...
		foreach($results as $result){
			...
			$xml .= '<video:description>' . stripcslashes($result['description']) . '</video:description>';
	 ...
	 function tab_linkvideo() {
    	...
    	<form name="addvideo" id="addvideo_form" method="POST"  action="<?php echo admin_url('admin.php?page=cvg-gallery-add').'#linkvideo'; ?>" accept-charset="utf-8" >
			...
			<tr valign="top"> 
				...
				<td><input type="text" size="35" name="videourl" value="" style="width:94%;"/><br />
						...
						jQuery("a[rel=fancy_cvg_gallery_slide_<?php echo $galleryID.'_'.$place_holder;?>]").fancybox({
							...
							'onComplete' : function() {
								...
								jwplayer('video_fancy_cvg_slide_gallery_<?php echo $galleryID.'_'.$place_holder;?>').setup({
									...
									"flashplayer" : "<?php echo $cool_video_gallery->plugin_url . "cvg-player/player.swf"; ?>",
